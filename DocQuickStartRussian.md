...
* [Быстрый старт](#user-content-Быстрый-старт)
    * [Основные функции QMS](#user-content-Основные-функции-qms)
    * [Загрузка файлов из Интернета](#user-content-Загрузка-файлов-из-Интернета)
    * [Установка основных компонентов](#user-content-Установка-основных-компонентов)
    * [Шаги после первого старта](#user-content-Шаги-после-первого-старта)
    * [Некоторые типичные операции QMS](#user-content-Некоторые-типичные-операции-qms)
        * [Изменение масштаба и перемещение карт](#user-content-Изменение-масштаба-и-перемещение-карт)
        * [Загрузить GPX файл и показать данные](#user-content-Загрузить-gpx-файл-и-показать-данные)
        * [Создать новую базу данных](#user-content-Создать-новую-базу-данных)
        * [Организация данных в базе данных](#user-content-Организация-данных-в-базе-данных)
        * [Создать трек](#user-content-Создать-трек)
        * [Показать информацию о треке](#user-content-Показать-информацию-о-треке)
        * [Создать трек (маршрут) через путевые точки](#user-content-Создать-трек-маршрут-через-путевые-точки)
        * [Поиск путевой точки](#user-content-Поиск-путевой-точки)
...
* _QMapShack_:
    ...
    * [Загрузить QMS с сервера](https://github.com/Maproom/qmapshack/releases "Загрузить QMS файл начальной установки").
      Загрузить файл `QMapShack_Win64bit_1.14.0.exe` с этого сайта (около 100 МБ, номер версии может отличаться).
...
* Найдите файл `QMapShack_Install_Windows64bit__1.14.0.exe` в папке загрузки. 
...
* **Подсказка:** *Никогда не используйте символы, отличные от ASCII, в именах каталогов установки!*  
...
* _Необязательно: Активировать цифровую модель рельефа:_    
    ...
    * Данные о высоте теперь доступны для выбранного региона (в примере юго-западный угол региона имеет координаты 59° северной широты, 30° восточной долготы и простирается на 1° в обоих направлениях)ю
...
* Выберите пункт меню `Файл - Загрузить данные ГИС` и выберите `QMS\GPX\МойПроект.gpx` в окне выбора файлов.
* В рабочей области появляется новый проект с именем `МойПроект`.
...
* Нажмите `A` на клавиатуре (это выбирает режим автоматической маршрутизации).
