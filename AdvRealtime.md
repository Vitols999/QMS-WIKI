...
Activate the OpenSky data source as described in the [previous section](#markdown-header-general-description).
...
* _Configuration of QMS_
    ...
    * Activate the GPS Tether data source as described at the [top of this page](#markdown-header-general-description).
...
The approach described in this section is similar to the one described in the section [Track replay](#markdown-header-track-replay). The only difference is the input to GPSBabel. For GPS replay a GPX file is used as GPSBabel input. Here, GPSBabel reads its input directly from the connected GPS receiver in form of NMEA data.
...
* check if your device is connected to a serial port. In this case, use the approach described in section [Using serial interface and Ncat](#markdown-header-using-serial-interface-and-ncat-windows-os),
...
After successful configuration of the TCP server and the QMS TCP client and after establishing the connection between server and client QMS receives permanently the GPS locations transmitted by the TCP server and visualizes them as moving objects (aircraft or circles) in the map window. If the record icon is pressed, then the locations are shown as an animated line in the map window. The locations can be recorded as described in the [OpenSky application section](#markdown-header-opensky-flight-data).
